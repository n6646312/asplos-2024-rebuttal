...
SYSCALL_DEFINE3(sys_update_zone_latency, unsigned long nid,
                                         unsigned long zid,
                                         unsigned long lat)
{
	struct pglist_data *node = NODE_DATA(nid);
	struct zone *_zone = &(node->node_zones[zid]);
	struct int old_lat = _zone->lat;
	struct int old_repr = node->repr_zone, i;
	...
	if (zid == old_repr) {
		if (lat > old_lat) {
			for (i = 0; i < MAX_NR_ZONES; i++) {
				if (node->node_zones[i].lat < lat) old_repr = i;
			}
		}
	} else {
		if (lat < node->node_zones[old_repr].lat) old_repr = zid;
	}

	if (old_repr != node->repr_zone) {
		for (i = 0; i < 3; i++) {
			sort(NODE_DATA(i)->affinity_list, MAX_NR_ZONES, sizeof(int));
		}
	}
